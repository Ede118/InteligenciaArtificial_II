...
OUTPUT_DIR = BASE_DIR / "evaluation_results" / "binary_sigmoid_up_down"
MODEL_PATH = BASE_DIR / "tensorflow_nn_binary_sigmoid_up_down.h5"
METADATA_PATH = BASE_DIR / "tensorflow_nn_binary_sigmoid_up_down_metadata.json"
...
def build_model():
    ...
    x = tf.keras.layers.Conv2D(16, (3, 3), activation="sigmoid")(image_input)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)
    x = tf.keras.layers.Conv2D(32, (3, 3), activation="sigmoid")(x)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)
    x = tf.keras.layers.Conv2D(64, (3, 3), activation="sigmoid")(x)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)
    x = tf.keras.layers.Flatten()(x)

    speed_input = tf.keras.Input(shape=(1,), name="speed")
    speed_branch = tf.keras.layers.Dense(16, activation="sigmoid")(speed_input)

    combined = tf.keras.layers.Concatenate()([x, speed_branch])
    combined = tf.keras.layers.Dense(128, activation="sigmoid")(combined)
...
def save_outputs(metrics, history, model, train_seconds, eval_seconds, original_counts, min_count, train_rows, test_rows):
    ...
    summary = {
        "variant_name": "binary_sigmoid_up_down",
    ...
    report_lines = [
        "Experimento binario up vs down con sigmoid",
