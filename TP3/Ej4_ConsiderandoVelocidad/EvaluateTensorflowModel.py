...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f"Ruta al json con metadata del modelo. Por defecto: {DEFAULT_MODEL_METADATA_PATH}",
    )
    parser.add_argument(
        "--test-dir",
    ...
    parser.add_argument(
        "--test-labels",
        default=DEFAULT_TEST_LABELS_PATH,
        help=f"CSV de labels del split de test. Por defecto: {DEFAULT_TEST_LABELS_PATH}",
    )
    parser.add_argument(
        "--output-dir",
...
def load_model_metadata(metadata_path):
    if not os.path.isfile(metadata_path):
        raise FileNotFoundError(
            f"No se encontro la metadata del modelo: {metadata_path}"
        )

    with open(metadata_path, "r", encoding="utf-8") as metadata_file:
        metadata = json.load(metadata_file)

    required_keys = {"class_folder_names", "image_size", "speed_min", "speed_max"}
    missing_keys = required_keys.difference(metadata.keys())
    if missing_keys:
        raise ValueError(
            "La metadata del modelo no tiene las claves necesarias: "
...
def load_test_rows(test_dir, labels_path, class_names):
    if not os.path.isfile(labels_path):
        raise FileNotFoundError(
            f"No se encontro el archivo de labels de test: {labels_path}"
        )
    ...
    if not rows:
        raise ValueError("No se encontraron muestras validas en el split de test.")

    return rows


def load_test_dataset(rows, class_names, image_size, speed_min, speed_max):
    images = []
    speeds = []
    labels = []
    file_paths = []
    ...
    for row in rows:
        images.append(
            load_and_preprocess_image(row["image_path"], image_size, normalize=True)
        )
    ...
    x_test_images = np.asarray(images, dtype=np.float32)
    x_test_speeds = np.asarray(speeds, dtype=np.float32)
    y_test = np.asarray(labels, dtype=np.int32)
    return x_test_images, x_test_speeds, y_test, file_paths
...
def compute_metrics(y_true, y_pred, probabilities, class_names):
    ...
    log_loss = float(
        tf.keras.losses.categorical_crossentropy(true_one_hot, probabilities).numpy().mean()
    )
...
def plot_misclassified_examples(
    file_paths, y_true, y_pred, class_names, image_size, output_dir, show_plots
):
    mistakes = [
...
def main():
    ...
    model_metadata = load_model_metadata(args.model_metadata)
    class_names = model_metadata["class_folder_names"]
    image_size = tuple(model_metadata["image_size"])
    speed_min = float(model_metadata["speed_min"])
    speed_max = float(model_metadata["speed_max"])

    if model.output_shape[-1] != len(class_names):
        raise ValueError(
            "La cantidad de clases del modelo no coincide con la metadata. "
            f"Salida del modelo: {model.output_shape[-1]}, clases detectadas: {len(class_names)}."
        )

    test_rows = load_test_rows(args.test_dir, args.test_labels, class_names)
    x_test_images, x_test_speeds, y_true, file_paths = load_test_dataset(
        test_rows,
        class_names,
        image_size,
        speed_min,
    ...
    plot_misclassified_examples(
        file_paths, y_true, y_pred, class_names, image_size, args.output_dir, args.show
    )
